...
describe('gemini.tsx main function kitty protocol', () => {
    ...
    it('should start normally with a warning when no sessions found for resume', async () => {
        const { SessionSelector, SessionError } = await import('./utils/sessionUtils.js');
        vi.mocked(SessionSelector).mockImplementation(() => ({
            resolveSession: vi
                .fn()
        ...
        expect(emitFeedbackSpy).not.toHaveBeenCalledWith('error', expect.stringContaining('Error resuming session'));
        processExitSpy.mockRestore();
        emitFeedbackSpy.mockRestore();
    });
