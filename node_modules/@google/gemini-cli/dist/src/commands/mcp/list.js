...
async function testMCPConnection(serverName, config, isTrusted, activeSettings) {
    ...
    const mcpContext = {
        ...
        emitMcpDiagnostic: (severity, message, error, serverName) => {
            ...
            if (severity === 'error') {
                debugLogger.error(chalk.red(`Error${serverName ? ` (${serverName})` : ''}: ${message}`), error);
            }
            else if (severity === 'warning') {
                debugLogger.warn(chalk.yellow(`Warning${serverName ? ` (${serverName})` : ''}: ${message}`), error);
            }
            else {
...
export async function listMcpServers(loadedSettingsArg) {
    ...
    for (const serverName of serverNames) {
        ...
        const status = await getServerStatus(serverName, server, loadedSettings.isTrusted, activeSettings);
        let statusIndicator = '';
        let statusText = '';
        switch (status) {
