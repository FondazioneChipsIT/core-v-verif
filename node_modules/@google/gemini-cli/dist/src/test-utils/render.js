...
class XtermStdout extends EventEmitter {
    ...
    lastRenderStaticContent = undefined;
    ...
    normalizeFrame = (text) => text.replace(/\r\n/g, '\n');
    generateSvg = () => generateSvgForTerminal(this.state.terminal);
    lastFrameRaw = (options = {}) => {
        ...
        if (normalized === '' && !options.allowEmpty) {
            throw new Error('lastFrameRaw() returned an empty string. If this is intentional, use lastFrameRaw({ allowEmpty: true }). ' +
    ...
    async waitUntilReady() {
        ...
        while (attempts < maxAttempts) {
            ...
            const expectedFrame = this.normalizeFrame(stripAnsi((this.lastRenderStaticContent ?? '') + (this.lastRenderOutput ?? ''))).trim();
...
const configProxy = new Proxy({}, {
    get(_target, prop) {
        if (prop === 'getTargetDir') {
            return () => path.join(path.parse(process.cwd()).root, 'Users', 'test', 'project', 'foo', 'bar', 'and', 'some', 'more', 'directories', 'to', 'make', 'it', 'long');
        }
