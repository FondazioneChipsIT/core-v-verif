...
describe('SessionSelector', () => {
    ...
    it('should throw SessionError with NO_SESSIONS_FOUND when resolving latest with no sessions', async () => {
        ...
        await expect(sessionSelector.resolveSession('latest')).rejects.toSatisfy((error) => {
            expect(error).toBeInstanceOf(SessionError);
            expect(error.code).toBe('NO_SESSIONS_FOUND');
            return true;
        });
    });
    ...
    it('should not list sessions marked as subagent', async () => {
        ...
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T10-00-${mainSessionId.slice(0, 8)}.json`), JSON.stringify(mainSession, null, 2));
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T11-00-${subagentSessionId.slice(0, 8)}.json`), JSON.stringify(subagentSession, null, 2));
        const sessionSelector = new SessionSelector(config);
        const sessions = await sessionSelector.listSessions();
