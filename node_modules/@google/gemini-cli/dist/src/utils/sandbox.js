...
export async function start_sandbox(config, nodeArgs = [], cliConfig, cliArgs = []) {
    ...
    try {
        ...
        if (proxyCommand) {
            ...
            if (proxy) {
                execSync(`${command} network inspect ${SANDBOX_NETWORK_NAME} || ${command} network create --internal ${SANDBOX_NETWORK_NAME}`);
                ...
                if (proxyCommand) {
                    execSync(`${command} network inspect ${SANDBOX_PROXY_NAME} || ${command} network create ${SANDBOX_PROXY_NAME}`);
                }
            }
        }
        ...
        if (proxyCommand) {
            ...
            const proxyContainerArgs = [
                ...
                ...parse(proxyCommand, process.env).filter((f) => typeof f === 'string'),
            ];
            ...
            proxyProcess.on('close', (code, signal) => {
                ...
                throw new FatalSandboxError(`Proxy container command '${command} ${proxyContainerArgs.join(' ')}' exited with code ${code}, signal ${signal}`);
            });
            debugLogger.log('waiting for proxy to start ...');
            await execAsync(`until timeout 0.25 curl -s http://localhost:8877; do sleep 0.25; done`);
            // connect proxy container to sandbox network
            // (workaround for older versions of docker that don't support multiple --network args)
            await execAsync(`${command} network connect ${SANDBOX_NETWORK_NAME} ${SANDBOX_PROXY_NAME}`);
        }
        // spawn child and let it inherit stdio
        process.stdin.pause();
...
async function start_lxc_sandbox(config, nodeArgs = [], cliArgs = []) {
    ...
    const workdir = path.resolve(process.cwd());
    debugLogger.log(`starting lxc sandbox (container: ${containerName}, workdir: ${workdir}) ...`);
    ...
    try {
        ...
        throw new FatalSandboxError(`Failed to query LXC container '${containerName}': ${err instanceof Error ? err.message : String(err)}. ` +
    ...
    if (!container) {
        throw new FatalSandboxError(`LXC container '${containerName}' not found. ` +
            `Create one with: lxc launch ubuntu:24.04 ${containerName}`);
    }
    if (container.status.toLowerCase() !== 'running') {
        throw new FatalSandboxError(`LXC container '${containerName}' is not running (current status: ${container.status}). ` +
    ...
    try {
        ...
        debugLogger.log(`mounted workspace '${workdir}' into container as device '${deviceName}'`);
    }
    catch (err) {
        throw new FatalSandboxError(`Failed to mount workspace into LXC container '${containerName}': ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        sandboxProcess.on('close', (code, signal) => {
            ...
            removeDevice();
            if (code !== 0 && code !== null) {
                debugLogger.log(`LXC sandbox process exited with code: ${code}, signal: ${signal}`);
            }
            resolve(code ?? 1);
        });
    });
