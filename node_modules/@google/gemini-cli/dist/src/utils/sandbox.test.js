...
describe('sandbox', () => {
    ...
    describe('start_sandbox', () => {
        ...
        describe('LXC sandbox', () => {
            ...
            it('should run lxc exec with correct args for a running container', async () => {
                ...
                const promise = start_sandbox(config, [], undefined, ['arg1']);
                await expect(promise).resolves.toBe(0);
                expect(spawn).toHaveBeenCalledWith('lxc', expect.arrayContaining(['exec', 'gemini-sandbox', '--cwd']), expect.objectContaining({ stdio: 'inherit' }));
            });
            it('should throw FatalSandboxError if lxc list fails', async () => {
                ...
                await expect(start_sandbox(config)).rejects.toThrow(/Failed to query LXC container/);
            });
    ...
    describe('gVisor (runsc)', () => {
        it('should use docker with --runtime=runsc on Linux', async () => {
            ...
            expect(spawn).toHaveBeenNthCalledWith(1, 'docker', expect.arrayContaining(['images', '-q', 'gemini-cli-sandbox']));
            // Verify docker run includes --runtime=runsc
            expect(spawn).toHaveBeenNthCalledWith(2, 'docker', expect.arrayContaining(['run', '--runtime=runsc']), expect.objectContaining({ stdio: 'inherit' }));
        });
    });
});
