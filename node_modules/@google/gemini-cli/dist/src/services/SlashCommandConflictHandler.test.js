...
describe('SlashCommandConflictHandler', () => {
     ...
     */
    const simulateEvent = (conflicts) => {
        const callback = vi
            .mocked(coreEvents.on)
            .mock.calls.find((call) => call[0] === CoreEvent.SlashCommandConflicts)[1];
        callback({ conflicts });
    };
    beforeEach(() => {
        vi.useFakeTimers();
    ...
    it('should listen for conflict events on start', () => {
        expect(coreEvents.on).toHaveBeenCalledWith(CoreEvent.SlashCommandConflicts, expect.any(Function));
    });
    it('should display a descriptive message for a single extension conflict', () => {
        ...
        expect(coreEvents.emitFeedback).toHaveBeenCalledWith('info', "Extension 'firebase' command '/deploy' was renamed to '/firebase.deploy' because it conflicts with built-in command.");
    });
    it('should display a descriptive message for a single MCP conflict', () => {
        ...
        vi.advanceTimersByTime(600);
        expect(coreEvents.emitFeedback).toHaveBeenCalledWith('info', "MCP server 'test-server' command '/pickle' was renamed to '/test-server.pickle' because it conflicts with extension 'pickle-rick' command.");
    });
    it('should group multiple conflicts for the same command name', () => {
        ...
        vi.advanceTimersByTime(600);
        expect(coreEvents.emitFeedback).toHaveBeenCalledWith('info', `Conflicts detected for command '/launch':
...
//# sourceMappingURL=SlashCommandConflictHandler.test.js.map