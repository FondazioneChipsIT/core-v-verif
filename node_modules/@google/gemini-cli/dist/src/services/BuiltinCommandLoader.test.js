...
describe('BuiltinCommandLoader', () => {
    ...
    describe('chat debug command', () => {
        it('should NOT add debug subcommand to chat/resume commands if not a nightly build', async () => {
            ...
            const resumeHasDebug = resumeCmd?.subCommands?.some((c) => c.name === 'debug') ?? false;
            expect(resumeHasDebug).toBe(false);
            const chatCheckpointsCmd = chatCmd?.subCommands?.find((c) => c.name === 'checkpoints');
            const chatCheckpointHasDebug = chatCheckpointsCmd?.subCommands?.some((c) => c.name === 'debug') ??
                false;
            expect(chatCheckpointHasDebug).toBe(false);
            const resumeCheckpointsCmd = resumeCmd?.subCommands?.find((c) => c.name === 'checkpoints');
            const resumeCheckpointHasDebug = resumeCheckpointsCmd?.subCommands?.some((c) => c.name === 'debug') ??
                false;
