...
export class ExtensionManager extends ExtensionLoader {
    ...
    async installOrUpdateExtension(installMetadata, previousExtensionConfig, requestConsentOverride) {
            ...
            if (installMetadata.type === 'git' ||
                ...
                if (!parsedGithubParts) {
                    ...
                    (result.failureReason === 'no release data' &&
                        ...
                        (await (requestConsentOverride ?? this.requestConsent)(`Error downloading github release for ${installMetadata.source} with the following error: ${result.errorMessage}.
...
Would you like to attempt to install via "git clone" instead?`))) {
            ...
            try {
                ...
                const previous = this.getExtensions().find((installed) => installed.name === previousName);
                const nameConflict = this.getExtensions().find((installed) => installed.name === newExtensionName &&
                    installed.name !== previousName);
                if (isUpdate && !previous) {
                    throw new Error(`Extension "${previousName}" was not already installed, cannot update it.`);
                }
                    ...
                    throw new Error(`Cannot update to "${newExtensionName}" because an extension with that name is already installed.`);
                }
                ...
                const isMigrating = Boolean(previous &&
                    previous.installMetadata &&
                    previous.installMetadata.source !== installMetadata.source);
                await maybeRequestConsentOrFail(newExtensionConfig, requestConsentOverride ?? this.requestConsent, newHasHooks, previousExtensionConfig, previousHasHooks, newSkills, previousSkills, isMigrating);
                ...
                if ((!isUpdate || newExtensionName !== previousName) &&
                    ...
                    throw new Error(`Cannot install extension "${newExtensionName}" because a directory with that name already exists. Please remove it manually.`);
                }
                ...
                if (isUpdate && previousExtensionConfig) {
                    const previousExtensionId = previous?.installMetadata
                        ...
                        : extensionId;
                    previousSettings = await getEnvContents(previousExtensionConfig, previousExtensionId, this.workspaceDir);
                    if (newExtensionName !== previousName) {
                        wasEnabledGlobally = this.extensionEnablementManager.isEnabled(previousName, homedir());
                        wasEnabledWorkspace = this.extensionEnablementManager.isEnabled(previousName, this.workspaceDir);
