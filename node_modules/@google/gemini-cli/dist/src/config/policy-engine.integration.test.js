...
describe('Policy Engine Integration Tests', () => {
    ...
    describe('Policy configuration produces valid PolicyEngine config', () => {
        ...
        it('should handle MCP server wildcard patterns correctly', async () => {
            ...
            // Tools from allowed server should be allowed
            expect((await engine.check({ name: 'mcp_allowed-server_tool1' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_allowed-server_another_tool' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            // Tools from trusted server should be allowed
            expect((await engine.check({ name: 'mcp_trusted-server_tool1' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_trusted-server_special_tool' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            // Tools from blocked server should be denied
            expect((await engine.check({ name: 'mcp_blocked-server_tool1' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
            expect((await engine.check({ name: 'mcp_blocked-server_any_tool' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
            // Tools from unknown servers should use default
            expect((await engine.check({ name: 'mcp_unknown-server_tool' }, undefined))
        ...
        it('should handle global MCP wildcard (*) in settings', async () => {
            ...
            expect((await engine.check({ name: 'mcp_mcp-server_tool' }, 'mcp-server'))
                .decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_another-server_tool' }, 'another-server')).decision).toBe(PolicyDecision.ALLOW);
        ...
        it('should correctly prioritize specific tool excludes over MCP server wildcards', async () => {
            ...
            // MCP server allowed (priority 4.1) provides general allow for server
            expect((await engine.check({ name: 'mcp_my-server_safe-tool' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            // But specific tool exclude (priority 4.4) wins over server allow
            expect((await engine.check({ name: 'mcp_my-server_dangerous-tool' }, undefined)).decision).toBe(PolicyDecision.DENY);
        });
        it('should handle complex mixed configurations', async () => {
            ...
            expect((await engine.check({ name: 'mcp_my-server_special-tool' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            // MCP server tools
            expect((await engine.check({ name: 'mcp_allowed-server_tool' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_trusted-server_tool' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_blocked-server_tool' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
        ...
        it('should verify priority ordering works correctly in practice', async () => {
            ...
            const blockedServerRule = rules.find((r) => r.toolName === 'mcp_blocked-server_*');
            ...
            const trustedServerRule = rules.find((r) => r.toolName === 'mcp_trusted-server_*');
            ...
            expect((await engine.check({ name: 'mcp_blocked-server_any' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
            expect((await engine.check({ name: 'specific-tool' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_trusted-server_any' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'mcp_mcp-server_any' }, undefined))
                .decision).toBe(PolicyDecision.ALLOW);
        ...
        it('should handle edge case: MCP server with both trust and exclusion', async () => {
            ...
            expect((await engine.check({ name: 'mcp_conflicted-server_tool' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
        });
        it('should handle edge case: specific tool allowed but server excluded', async () => {
            ...
            expect((await engine.check({ name: 'mcp_my-server_special-tool' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
            expect((await engine.check({ name: 'mcp_my-server_other-tool' }, undefined))
                .decision).toBe(PolicyDecision.DENY);
        });
