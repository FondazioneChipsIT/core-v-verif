 ...
 */
export async function promptForConsentNonInteractive(prompt, defaultValue = true) {
 ...
 * extensionConfig.
 */
async function extensionConsentString(extensionConfig, hasHooks, skills = [], previousName, wasMigrated) {
    const sanitizedConfig = escapeAnsiCtrlCodes(extensionConfig);
    const output = [];
    const mcpServerEntries = Object.entries(sanitizedConfig.mcpServers || {});
    if (wasMigrated) {
        if (previousName && previousName !== sanitizedConfig.name) {
            output.push(`Migrating extension "${previousName}" to a new repository, renaming to "${sanitizedConfig.name}", and installing updates.`);
        }
        else {
            output.push(`Migrating extension "${sanitizedConfig.name}" to a new repository and installing updates.`);
        }
    }
    else if (previousName && previousName !== sanitizedConfig.name) {
        output.push(`Renaming extension "${previousName}" to "${sanitizedConfig.name}" and installing updates.`);
    }
    else {
 ...
 */
export async function maybeRequestConsentOrFail(extensionConfig, requestConsent, hasHooks, previousExtensionConfig, previousHasHooks, skills = [], previousSkills = [], isMigrating = false) {
    const extensionConsent = await extensionConsentString(extensionConfig, hasHooks, skills, previousExtensionConfig?.name, isMigrating);
