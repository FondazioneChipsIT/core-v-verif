...
import { requestConsentNonInteractive, requestConsentInteractive, maybeRequestConsentOrFail, } from './consent.js';
...
async function expectConsentSnapshot(consentString) {
 ...
 */
function normalizePathsForSnapshot(str, tempDir) {
...
describe('consent', () => {
    ...
    describe('maybeRequestConsentOrFail', () => {
        ...
        describe('consent string generation', () => {
            it('should generate a consent string with all fields', async () => {
                ...
                expect(requestConsent).toHaveBeenCalledTimes(1);
                const consentString = requestConsent.mock.calls[0][0];
            ...
            it('should request consent if extension is migrated', async () => {
                const requestConsent = vi.fn().mockResolvedValue(true);
                await maybeRequestConsentOrFail(baseConfig, requestConsent, false, { ...baseConfig, name: 'old-ext' }, false, [], [], true);
                expect(requestConsent).toHaveBeenCalledTimes(1);
                let consentString = requestConsent.mock.calls[0][0];
    ...
    describe('skillsConsentString', () => {
        it('should generate a consent string for skills', async () => {
            ...
            let consentString = await skillsConsentString([skill1], 'https://example.com/repo.git', '/mock/target/dir');
            consentString = normalizePathsForSnapshot(consentString, tempDir);
            await expectConsentSnapshot(consentString);
        });
    });
