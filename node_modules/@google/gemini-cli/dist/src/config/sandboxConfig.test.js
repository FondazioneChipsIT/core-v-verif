...
describe('loadSandboxConfig', () => {
    ...
    describe('with GEMINI_SANDBOX environment variable', () => {
        ...
        it('should throw if GEMINI_SANDBOX is an invalid command', async () => {
            ...
            await expect(loadSandboxConfig({}, {})).rejects.toThrow("Invalid sandbox command 'invalid-command'. Must be one of docker, podman, sandbox-exec, runsc, lxc");
        });
        ...
        it('should throw if GEMINI_SANDBOX=lxc but lxc command does not exist', async () => {
            process.env['GEMINI_SANDBOX'] = 'lxc';
            mockedCommandExistsSync.mockReturnValue(false);
            await expect(loadSandboxConfig({}, {})).rejects.toThrow("Missing sandbox command 'lxc' (from GEMINI_SANDBOX)");
        });
    });
    ...
    describe("with sandbox: 'command'", () => {
        ...
        it('should throw if the specified command is invalid', async () => {
            await expect(loadSandboxConfig({}, { sandbox: 'invalid-command' })).rejects.toThrow("Invalid sandbox command 'invalid-command'. Must be one of docker, podman, sandbox-exec, runsc, lxc");
        });
    });
    ...
    describe('with sandbox: runsc (gVisor)', () => {
        ...
        it('should prioritize GEMINI_SANDBOX over CLI and settings', async () => {
            process.env['GEMINI_SANDBOX'] = 'runsc';
            const config = await loadSandboxConfig({ tools: { sandbox: 'docker' } }, { sandbox: 'podman' });
            expect(config).toEqual({ command: 'runsc', image: 'default/image' });
        });
        it('should reject runsc on macOS (Linux-only)', async () => {
            mockedOsPlatform.mockReturnValue('darwin');
            await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow('gVisor (runsc) sandboxing is only supported on Linux');
        });
        ...
        it('should throw if runsc binary not found', async () => {
            mockedCommandExistsSync.mockReturnValue(false);
            await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow("Missing sandbox command 'runsc' (from GEMINI_SANDBOX)");
        });
        it('should throw if Docker not available (runsc requires Docker)', async () => {
            ...
            await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow("runsc (gVisor) requires Docker. Install Docker, or use sandbox: 'docker'.");
        });
        it('should NOT auto-detect runsc when both runsc and docker available', async () => {
            mockedCommandExistsSync.mockImplementation((cmd) => cmd === 'runsc' || cmd === 'docker');
            const config = await loadSandboxConfig({}, { sandbox: true });
