...
import { DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_MODEL_AUTO, DEFAULT_GEMINI_FLASH_MODEL, DEFAULT_GEMINI_FLASH_LITE_MODEL, PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_3_1_MODEL, PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL, PREVIEW_GEMINI_FLASH_MODEL, PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL, AuthType, UserTierId, } from '@google/gemini-cli-core';
// Mock dependencies
...
describe('<ModelDialog />', () => {
    ...
    const renderComponent = async (configValue = mockConfig, authType = AuthType.LOGIN_WITH_GOOGLE) => {
        ...
        const result = renderWithProviders(_jsx(ModelDialog, { onClose: mockOnClose }), {
            config: configValue,
            settings,
        });
    ...
    it('renders the "manual" view initially for users with no pro access and filters Pro models with correct order', async () => {
        ...
        mockGetDisplayString.mockImplementation((val) => val);
        const { lastFrame, unmount } = await renderComponent();
        ...
        const flashLitePreviewIdx = output.indexOf(PREVIEW_GEMINI_3_1_FLASH_LITE_MODEL);
    ...
    it('switches to "manual" view when "Manual" is selected and uses getDisplayString for models', async () => {
        mockGetDisplayString.mockImplementation((val) => {
            if (val === DEFAULT_GEMINI_MODEL)
                return 'Formatted Pro Model';
            if (val === DEFAULT_GEMINI_FLASH_MODEL)
                return 'Formatted Flash Model';
    ...
    it('shows the preferred manual model in the main view option using getDisplayString', async () => {
        ...
        mockGetDisplayString.mockImplementation((val) => {
            if (val === DEFAULT_GEMINI_MODEL)
                return 'My Custom Model Display';
            if (val === 'auto-gemini-2.5')
                return 'Auto (Gemini 2.5)';
            return val;
        });
    ...
    describe('Preview Models', () => {
        ...
        it('shows Gemini 3.1 models in manual view when Gemini 3.1 IS launched', async () => {
            ...
            const { lastFrame, stdin, waitUntilReady, unmount } = await renderComponent(mockConfig, AuthType.USE_VERTEX_AI);
            // Go to manual view
            await act(async () => {
                stdin.write('\u001B[B'); // Manual
            });
        ...
        it('uses custom tools model when Gemini 3.1 IS launched and auth is Gemini API Key', async () => {
            mockGetGemini31LaunchedSync.mockReturnValue(true);
            const { stdin, waitUntilReady, unmount } = await renderComponent(mockConfig, AuthType.USE_GEMINI);
            // Go to manual view
            await act(async () => {
                stdin.write('\u001B[B'); // Manual
            });
