...
export const RewindConfirmation = ({ stats, onConfirm, terminalWidth, timestamp, }) => {
    ...
    if (isScreenReaderEnabled) {
        return (_jsxs(Box, { flexDirection: "column", width: terminalWidth, children: [_jsx(Text, { bold: true, children: "Confirm Rewind" }), stats && (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { children: stats.fileCount === 1
                                ? `File: ${stats.details?.at(0)?.fileName}`
                                : `${stats.fileCount} files affected` }), _jsxs(Text, { children: ["Lines added: ", stats.addedLines] }), _jsxs(Text, { children: ["Lines removed: ", stats.removedLines] }), timestamp && _jsxs(Text, { children: ["(", formatTimeAgo(timestamp), ")"] }), _jsx(Text, { children: "Note: Rewinding does not affect files edited manually or by the shell tool." })] })), !stats && (_jsxs(Box, { children: [_jsx(Text, { color: theme.text.secondary, children: "No code changes to revert." }), timestamp && (_jsxs(Text, { color: theme.text.secondary, children: [' ', "(", formatTimeAgo(timestamp), ")"] }))] })), _jsx(Text, { children: "Select an action:" }), _jsx(Text, { color: theme.text.secondary, children: "Use arrow keys to navigate, Enter to confirm, Esc to cancel." }), _jsx(RadioButtonSelect, { items: options, onSelect: handleSelect, isFocused: true })] }));
    }
