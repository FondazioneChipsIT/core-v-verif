...
describe('<HistoryItemDisplay />', () => {
    ...
    it('renders UserMessage for "user" type', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: item }));
    ...
    it.each([true, false])('renders InfoMessage for "info" type with multi-line text (alternateBuffer=%s)', async (useAlternateBuffer) => {
        const item = {
            ...baseItem,
            type: MessageType.INFO,
            text: '⚡ Line 1\n⚡ Line 2\n⚡ Line 3',
        };
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: item }), { useAlternateBuffer });
        await waitUntilReady();
    ...
    it('renders StatsDisplay for "stats" type', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(SessionStatsProvider, { children: _jsx(HistoryItemDisplay, { ...baseItem, item: item }) }));
        await waitUntilReady();
    ...
    it('should escape ANSI codes in text content', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { item: historyItem, terminalWidth: 80, isPending: false }));
        await waitUntilReady();
    ...
    it('should escape ANSI codes in tool confirmation details', async () => {
        ...
        const { waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { item: historyItem, terminalWidth: 80, isPending: false }));
        await waitUntilReady();
    ...
    describe('thinking items', () => {
        ...
        it('renders "Thinking..." header when isFirstThinking is true', async () => {
            ...
            const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: item, isFirstThinking: true }), {
                settings: createMockSettings({
                    merged: { ui: { inlineThinkingMode: 'full' } },
                }),
            });
    ...
    describe.each([true, false])('gemini items (alternateBuffer=%s)', (useAlternateBuffer) => {
        ...
        it('should render a truncated gemini item', async () => {
            ...
            const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { item: item, isPending: false, terminalWidth: 80, availableTerminalHeight: 10 }), { useAlternateBuffer });
            await waitUntilReady();
            expect(lastFrame()).toMatchSnapshot();
            unmount();
        });
        it('should render a full gemini item when using availableTerminalHeightGemini', async () => {
            ...
            const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HistoryItemDisplay, { item: item, isPending: false, terminalWidth: 80, availableTerminalHeight: 10, availableTerminalHeightGemini: Number.MAX_SAFE_INTEGER }), { useAlternateBuffer });
            await waitUntilReady();
            expect(lastFrame()).toMatchSnapshot();
            unmount();
        });
