...
- Load testing for session management`;
    ...
    const renderDialog = (options) => renderWithProviders(_jsx(ExitPlanModeDialog, { planPath: mockPlanFullPath, onApprove: onApprove, onFeedback: onFeedback, onCancel: onCancel, getPreferredEditor: vi.fn(), width: 80, availableHeight: 24 }), {
        ...options,
        config: {
    ...
    describe.each([{ useAlternateBuffer: true }, { useAlternateBuffer: false }])('useAlternateBuffer: $useAlternateBuffer', ({ useAlternateBuffer }) => {
        ...
        it('bubbles up Ctrl+C when feedback is empty while editing', async () => {
            ...
            const { stdin, lastFrame } = renderWithProviders(_jsx(BubbleListener, { children: _jsx(ExitPlanModeDialog, { planPath: mockPlanFullPath, onApprove: onApprove, onFeedback: onFeedback, onCancel: onCancel, getPreferredEditor: vi.fn(), width: 80, availableHeight: 24 }) }), {
                useAlternateBuffer,
                config: {
                    getTargetDir: () => mockTargetDir,
                    getIdeMode: () => false,
        ...
        it('automatically submits feedback when Ctrl+X is used to edit the plan', async () => {
            ...
            await waitFor(() => {
                expect(onFeedback).toHaveBeenCalledWith('I have edited the plan or annotated it with feedback. Review the edited plan, update if necessary, and present it again for approval.');
            });
        });
    });
});
