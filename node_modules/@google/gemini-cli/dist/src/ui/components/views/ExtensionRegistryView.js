...
export function ExtensionRegistryView({ onSelect, onClose, extensionManager, }) {
    ...
    const { extensions, loading, error, search } = useExtensionRegistry('', config.getExtensionRegistryURI());
    ...
    const [installedExtensions, setInstalledExtensions] = useState(() => extensionManager.getExtensions());
    ...
    const handleInstall = useCallback(async (extension, requestConsentOverride) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Box, { display: selectedExtension ? 'none' : 'flex', flexDirection: "column", width: "100%", height: "100%", children: _jsx(SearchableList, { title: "Extensions", items: items, onSelect: handleSelect, onClose: onClose || (() => { }), searchPlaceholder: "Search extension gallery", renderItem: renderItem, header: header, footer: footer, maxItemsToShow: maxItemsToShow, useSearch: useRegistrySearch, onSearch: search, resetSelectionOnItemsChange: true, isFocused: !selectedExtension }) }), selectedExtension && (_jsx(ExtensionDetails, { extension: selectedExtension, onBack: handleBack, onInstall: async (requestConsentOverride) => {
                    await handleInstall(selectedExtension, requestConsentOverride);
                }, isInstalled: installedExtensions.some((e) => e.name === selectedExtension.extensionName) }))] }));
}
