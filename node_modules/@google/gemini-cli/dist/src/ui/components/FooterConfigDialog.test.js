...
describe('<FooterConfigDialog />', () => {
    ...
    it('renders correctly with default settings', async () => {
        ...
        const renderResult = renderWithProviders(_jsx(FooterConfigDialog, { onClose: mockOnClose }), { settings });
    ...
    it('toggles an item when enter is pressed', async () => {
        const settings = createMockSettings();
        const { lastFrame, stdin, waitUntilReady } = renderWithProviders(_jsx(FooterConfigDialog, { onClose: mockOnClose }), { settings });
    ...
    it('closes on Esc', async () => {
        const settings = createMockSettings();
        const { stdin, waitUntilReady } = renderWithProviders(_jsx(FooterConfigDialog, { onClose: mockOnClose }), { settings });
        await waitUntilReady();
        act(() => {
            stdin.write('\x1b'); // Esc
        });
    ...
    it('updates the preview when Show footer labels is toggled off', async () => {
        ...
        await waitFor(() => {
            ...
            const previewLine = lastFrame()
                .split('\n')
                .find((line) => line.includes('Preview:')) || '';
            const nextLine = lastFrame().split('\n')[lastFrame().split('\n').indexOf(previewLine) + 1] || '';
...
//# sourceMappingURL=FooterConfigDialog.test.js.map