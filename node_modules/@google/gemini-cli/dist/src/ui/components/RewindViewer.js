...
export const RewindViewer = ({ conversation, onExit, onRewind, }) => {
    ...
    if (isScreenReaderEnabled) {
        return (_jsxs(Box, { flexDirection: "column", width: terminalWidth, children: [_jsx(Text, { bold: true, children: "Rewind - Select a conversation point:" }), _jsx(BaseSelectionList, { items: items, initialIndex: items.length - 1, isFocused: true, showNumbers: true, wrapAround: false, onSelect: (item) => {
                        if (item?.id) {
                        ...
                        const text = item.id === 'current-position'
                            ? 'Stay at current position'
                            : getCleanedRewindText(item);
                        return _jsx(Text, { children: text });
                    } }), _jsx(Text, { color: theme.text.secondary, children: "Press Esc to exit, Enter to select, arrow keys to navigate." })] }));
    }
