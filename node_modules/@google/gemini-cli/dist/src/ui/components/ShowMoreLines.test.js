...
describe('ShowMoreLines', () => {
    ...
    ])('renders nothing when: overflow=%s, streaming=%s, constrain=%s', async (overflowingIds, streamingState, constrainHeight) => {
        mockUseOverflowState.mockReturnValue({ overflowingIds });
        mockUseStreamingContext.mockReturnValue(streamingState);
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(ShowMoreLines, { constrainHeight: constrainHeight }));
    ...
    it('renders message in STANDARD mode when overflowing', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(ShowMoreLines, { constrainHeight: true }));
        await waitUntilReady();
    ...
    ])('renders message in ASB mode when overflowing and state is %s', async (streamingState) => {
    ...
    it('renders message in ASB mode when isOverflowing prop is true even if internal overflow state is empty', async () => {
        ...
        mockUseStreamingContext.mockReturnValue(StreamingState.Idle);
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(ShowMoreLines, { constrainHeight: true, isOverflowing: true }));
        await waitUntilReady();
        expect(lastFrame().toLowerCase()).toContain('press ctrl+o to show more lines');
        unmount();
    });
    it('renders nothing when isOverflowing prop is false even if internal overflow state has IDs', async () => {
        ...
        mockUseStreamingContext.mockReturnValue(StreamingState.Idle);
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(ShowMoreLines, { constrainHeight: true, isOverflowing: false }));
        await waitUntilReady();
        expect(lastFrame({ allowEmpty: true })).toBe('');
        unmount();
    });
