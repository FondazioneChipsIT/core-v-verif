...
describe('SessionBrowser component', () => {
    ...
    it('shows empty state when no sessions exist', async () => {
        ...
        const onExit = vi.fn();
        const { lastFrame, waitUntilReady } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [] }));
    ...
    it('renders a list of sessions and marks current session as disabled', async () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { lastFrame, waitUntilReady } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [session1, session2] }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
    });
    it('enters search mode, filters sessions, and renders match snippets', async () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { lastFrame, waitUntilReady } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [searchSession, otherSession] }));
        await waitUntilReady();
    ...
    it('does not allow resuming or deleting the current session', async () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { waitUntilReady } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [currentSession, otherSession] }));
        await waitUntilReady();
    ...
    it('shows an error state when loading sessions fails', async () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { lastFrame, waitUntilReady } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testError: "storage failure" }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
    });
});
