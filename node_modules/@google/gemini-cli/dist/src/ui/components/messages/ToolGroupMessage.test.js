...
describe('<ToolGroupMessage />', () => {
    ...
    describe('Golden Snapshots', () => {
        ...
        it('hides confirming tools (standard behavior)', async () => {
            ...
            const item = createItem(toolCalls);
            const { lastFrame, unmount, waitUntilReady } = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, item: item, toolCalls: toolCalls }), { config: baseMockConfig, settings: fullVerbositySettings });
    ...
    describe('Ask User Filtering', () => {
        ...
        it('renders nothing when only tool is in-progress AskUser with borderBottom=false', async () => {
            ...
            const item = createItem(toolCalls);
            const { lastFrame, unmount, waitUntilReady } = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, item: item, toolCalls: toolCalls, borderBottom: false }), { config: baseMockConfig, settings: fullVerbositySettings });
        ...
        it('does not render a bottom-border fragment when all tools are filtered out', async () => {
            ...
            const item = createItem(toolCalls);
            const { lastFrame, unmount, waitUntilReady } = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, item: item, toolCalls: toolCalls, borderTop: false, borderBottom: true }), {
                config: baseMockConfig,
        ...
        it('does not leave a border stub when transitioning from visible to fully filtered tools', async () => {
            ...
            const firstRender = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, item: initialItem, toolCalls: visibleTools, borderTop: false, borderBottom: true }), {
                config: baseMockConfig,
                settings: lowVerbositySettings,
            });
            ...
            firstRender.unmount();
            const secondRender = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, item: hiddenItem, toolCalls: hiddenTools, borderTop: false, borderBottom: true }), {
                config: baseMockConfig,
                settings: lowVerbositySettings,
            });
        ...
        it('keeps visible tools rendered with many filtered tools (stress case)', async () => {
            ...
            const hiddenTools = Array.from({ length: 50 }, (_, index) => createToolCall({
                callId: `hidden-${index}`,
        ...
        it('renders explicit closing slice even at very narrow terminal width', async () => {
            const toolCalls = [];
            const item = createItem(toolCalls);
            const { lastFrame, unmount, waitUntilReady } = renderWithProviders(_jsx(ToolGroupMessage, { item: item, toolCalls: toolCalls, terminalWidth: 8, borderTop: false, borderBottom: true }), {
                config: baseMockConfig,
                settings: fullVerbositySettings,
            });
            await waitUntilReady();
