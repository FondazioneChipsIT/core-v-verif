...
export const FocusHint = ({ shouldShowFocusHint, isThisShellFocused }) => {
    ...
    return (_jsx(Box, { marginLeft: 1, flexShrink: 0, children: _jsx(Text, { color: isThisShellFocused ? theme.ui.focus : theme.ui.active, children: isThisShellFocused
                ? `(${formatCommand(Command.UNFOCUS_SHELL_INPUT)} to unfocus)`
                : `(${formatCommand(Command.FOCUS_SHELL_INPUT)} to focus)` }) }));
};
export const ToolStatusIndicator = ({ status: coreStatus, name, isFocused, }) => {
    ...
    return (_jsxs(Box, { minWidth: STATUS_INDICATOR_WIDTH, children: [status === ToolCallStatus.Pending && (_jsx(Text, { color: theme.status.success, children: TOOL_STATUS.PENDING })), status === ToolCallStatus.Executing && (_jsx(Text, { color: statusColor, children: _jsx(CliSpinner, { type: "toggle" }) })), status === ToolCallStatus.Success && (_jsx(Text, { color: theme.status.success, "aria-label": 'Success:', children: TOOL_STATUS.SUCCESS })), status === ToolCallStatus.Confirming && (_jsx(Text, { color: statusColor, "aria-label": 'Confirming:', children: TOOL_STATUS.CONFIRMING })), status === ToolCallStatus.Canceled && (_jsx(Text, { color: statusColor, "aria-label": 'Canceled:', bold: true, children: TOOL_STATUS.CANCELED })), status === ToolCallStatus.Error && (_jsx(Text, { color: theme.status.error, "aria-label": 'Error:', bold: true, children: TOOL_STATUS.ERROR }))] }));
};
export const ToolInfo = ({ name, description, status: coreStatus, emphasis, progressMessage: _progressMessage, originalRequestName, }) => {
    const status = mapCoreStatusToDisplayStatus(coreStatus);
