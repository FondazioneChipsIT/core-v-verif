...
const REDIRECTION_WARNING_NOTE_TEXT = 'Command contains redirection which can be undesirable.';
const REDIRECTION_WARNING_TIP_LABEL = 'Tip:  '; // Padded to align with "Note: "
export const ToolConfirmationMessage = ({ callId, confirmationDetails, config, getPreferredEditor, isFocused = true, availableTerminalHeight, terminalWidth, }) => {
    ...
    const { question, bodyContent, options, securityWarnings, initialIndex } = useMemo(() => {
        ...
        if (isTrustedFolder && allowPermanentApproval) {
            ...
            const isSafeToPersist = confirmationDetails.type === 'info' ||
        ...
        if (confirmationDetails.type === 'exit_plan_mode') {
            bodyContent = (_jsx(ExitPlanModeDialog, { planPath: confirmationDetails.planPath, getPreferredEditor: getPreferredEditor, onApprove: (approvalMode) => {
        ...
        if (confirmationDetails.type === 'edit') {
            ...
            bodyContent = (_jsxs(Box, { flexDirection: "column", children: [_jsx(MaxSizedBox, { maxHeight: bodyContentHeight, maxWidth: Math.max(terminalWidth, 1), children: _jsx(Box, { flexDirection: "column", children: commandsToDisplay.map((cmd, idx) => (_jsx(Box, { flexDirection: "column", paddingBottom: idx < commandsToDisplay.length - 1 ? 1 : 0, children: colorizeCode({
                                    code: cmd,
                                    language: 'bash',
