...
describe('BaseSelectionList', () => {
    ...
    const renderComponent = async (props = {}, activeIndex = 0) => {
        ...
        const result = renderWithProviders(_jsx(BaseSelectionList, { ...defaultProps }));
    ...
    describe('Scrolling and Pagination (maxItemsToShow)', () => {
        ...
        const MAX_ITEMS = 3;
        const renderScrollableList = async (initialActiveIndex = 0) => {
            ...
            mockRenderItem.mockImplementation((item, context) => (_jsx(Text, { color: context.titleColor, children: item.label })));
            const { rerender, lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(BaseSelectionList, { ...componentProps }));
        ...
        it('should handle maxItemsToShow larger than the list length', async () => {
            const { lastFrame, unmount } = await renderComponent({ items: longList, maxItemsToShow: 15 }, 0);
            const output = lastFrame();
    ...
    describe('Scroll Arrows (showScrollArrows)', () => {
        ...
        it('should show arrows and correct items when scrolled to the middle', async () => {
            const { lastFrame, unmount } = await renderComponent({ items: longList, maxItemsToShow: MAX_ITEMS, showScrollArrows: true }, 5);
            expect(lastFrame()).toMatchSnapshot();
            unmount();
        });
        it('should show arrows and correct items when scrolled to the end', async () => {
            const { lastFrame, unmount } = await renderComponent({ items: longList, maxItemsToShow: MAX_ITEMS, showScrollArrows: true }, 9);
            expect(lastFrame()).toMatchSnapshot();
            unmount();
        });
