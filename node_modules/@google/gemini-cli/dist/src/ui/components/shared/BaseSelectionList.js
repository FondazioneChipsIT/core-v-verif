 ...
 */
export function BaseSelectionList({ items, initialIndex = 0, onSelect, onHighlight, isFocused = true, showNumbers = true, showScrollArrows = false, maxItemsToShow = 10, wrapAround = true, focusKey, priority, selectedIndicator = '●', renderItem, }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [showScrollArrows && items.length > maxItemsToShow && (_jsx(Text, { color: scrollOffset > 0 ? theme.text.primary : theme.text.secondary, children: "\u25B2" })), visibleItems.map((item, index) => {
                ...
                return (_jsxs(Box, { alignItems: "flex-start", backgroundColor: isSelected ? theme.background.focus : undefined, children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isSelected ? theme.ui.focus : theme.text.primary, "aria-hidden": true, children: isSelected ? selectedIndicator : ' ' }) }), showNumbers && !item.hideNumber && (_jsx(Box, { marginRight: 1, flexShrink: 0, minWidth: itemNumberText.length, "aria-state": { checked: isSelected }, children: _jsx(Text, { color: numberColor, children: itemNumberText }) })), _jsx(Box, { flexGrow: 1, children: renderItem(item, {
                                isSelected,
                                titleColor,
                                numberColor,
                            }) })] }, item.key));
