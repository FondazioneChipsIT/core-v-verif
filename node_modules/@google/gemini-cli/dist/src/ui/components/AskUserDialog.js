...
const ChoiceQuestionView = ({ question, onAnswer, onSelectionChange, onEditingCustomOption, availableWidth, availableHeight, initialAnswer, progressHeader, keyboardHints, }) => {
    ...
    const buildAnswerString = useCallback((indices, includeCustomOption, customOption) => {
        ...
        if (includeCustomOption && customOption.trim()) {
            const expanded = expandPastePlaceholders(customOption, customBuffer.pastedContent);
        ...
        if (question.multiSelect) {
            ...
            if (itemValue.type === 'option') {
                ...
                if (customOptionText.trim()) {
                    onAnswer(expandPastePlaceholders(customOptionText, customBuffer.pastedContent).trim());
                }
            }
        }
    }, [
    ...
    const listHeight = availableHeight
        ? Math.max(1, availableHeight - overhead)
        : undefined;
    const questionHeightLimit = listHeight && !isAlternateBuffer
        ...
        : undefined;
    const maxItemsToShow = listHeight && questionHeightLimit
        ? Math.max(1, Math.floor((listHeight - questionHeightLimit) / 2))
        : selectionItems.length;
    return (_jsxs(Box, { flexDirection: "column", children: [progressHeader, _jsx(Box, { marginBottom: TITLE_MARGIN, children: _jsx(MaxSizedBox, { maxHeight: questionHeightLimit, maxWidth: availableWidth, overflowDirection: "bottom", children: _jsxs(Box, { flexDirection: "column", children: [_jsx(MarkdownDisplay, { text: autoBoldIfPlain(question.question), terminalWidth: availableWidth - DIALOG_PADDING, isPending: false }), question.multiSelect && (_jsx(Text, { color: theme.text.secondary, italic: true, children: "(Select all that apply)" }))] }) }) }), _jsx(BaseSelectionList, { items: selectionItems, onSelect: handleSelect, onHighlight: handleHighlight, focusKey: isCustomOptionFocused ? 'other' : undefined, maxItemsToShow: maxItemsToShow, showScrollArrows: true, renderItem: (item, context) => {
