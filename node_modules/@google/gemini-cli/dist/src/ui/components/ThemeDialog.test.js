...
describe('ThemeDialog Snapshots', () => {
    ...
    it.each([true, false])('should render correctly in theme selection mode (isDevelopment: %s)', async (isDev) => {
        mockIsDevelopment.value = isDev;
        const settings = createMockSettings();
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(ThemeDialog, { ...baseProps, settings: settings }), { settings });
    ...
    it('should render correctly in scope selector mode', async () => {
        const settings = createMockSettings();
        const { lastFrame, stdin, waitUntilReady, unmount } = renderWithProviders(_jsx(ThemeDialog, { ...baseProps, settings: settings }), { settings });
        await waitUntilReady();
    ...
    it('should call onCancel when ESC is pressed', async () => {
        ...
        const settings = createMockSettings();
        const { stdin, waitUntilReady, unmount } = renderWithProviders(_jsx(ThemeDialog, { ...baseProps, onCancel: mockOnCancel, settings: settings }), { settings });
        await waitUntilReady();
        await act(async () => {
            stdin.write('\x1b');
        });
    ...
    it('should call onSelect when a theme is selected', async () => {
        const settings = createMockSettings();
        const { stdin, waitUntilReady, unmount } = renderWithProviders(_jsx(ThemeDialog, { ...baseProps, settings: settings }), {
            settings,
        });
        await waitUntilReady();
...
describe('Initial Theme Selection', () => {
    ...
    it('should default to a light theme when terminal background is light and no theme is set', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(ThemeDialog, { ...baseProps, settings: settings }), {
            settings,
