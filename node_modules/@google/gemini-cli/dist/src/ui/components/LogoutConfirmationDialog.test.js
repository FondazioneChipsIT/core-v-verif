...
describe('LogoutConfirmationDialog', () => {
    ...
    it('should render the dialog with title, description, and hint', async () => {
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(LogoutConfirmationDialog, { onSelect: vi.fn() }));
        await waitUntilReady();
        expect(lastFrame()).toContain('You are now signed out');
        expect(lastFrame()).toContain('Sign in again to continue using Gemini CLI, or exit the application.');
    ...
    it('should render RadioButtonSelect with Login and Exit options', async () => {
        const { waitUntilReady, unmount } = renderWithProviders(_jsx(LogoutConfirmationDialog, { onSelect: vi.fn() }));
        await waitUntilReady();
    ...
    it('should call onSelect with LOGIN when Login is selected', async () => {
        ...
        const { waitUntilReady, unmount } = renderWithProviders(_jsx(LogoutConfirmationDialog, { onSelect: onSelect }));
        await waitUntilReady();
        const mockCall = vi.mocked(RadioButtonSelect).mock.calls[0][0];
        await act(async () => {
    ...
    it('should call onSelect with EXIT when escape key is pressed', async () => {
        const onSelect = vi.fn();
        const { stdin, waitUntilReady, unmount } = renderWithProviders(_jsx(LogoutConfirmationDialog, { onSelect: onSelect }));
        await waitUntilReady();
        await act(async () => {
