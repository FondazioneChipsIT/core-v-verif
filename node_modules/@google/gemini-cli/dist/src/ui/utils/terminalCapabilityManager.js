...
export class TerminalCapabilityManager {
    ...
    supportsOsc9Notifications(env = process.env) {
        if (env['WT_SESSION']) {
            return false;
        }
        return (this.hasOsc9TerminalSignature(this.getTerminalName()) ||
            this.hasOsc9TerminalSignature(env['TERM_PROGRAM']) ||
            this.hasOsc9TerminalSignature(env['TERM']));
    }
    hasOsc9TerminalSignature(value) {
        if (!value) {
            return false;
        }
        const normalized = value.toLowerCase();
        return (normalized.includes('wezterm') ||
