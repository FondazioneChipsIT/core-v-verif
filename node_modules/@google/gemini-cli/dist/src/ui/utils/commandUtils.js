 ...
 */
export const isAtCommand = (query) => AT_COMMAND_DETECT_REGEX.test(query);
/**
 * Checks if a query string potentially represents an '/' command.
...
const shouldUseOsc52 = (tty, settings) => Boolean(tty) &&
    !isDumbTerm() &&
...
export const copyToClipboard = async (text, settings) => {
    if (!text)
        return;
