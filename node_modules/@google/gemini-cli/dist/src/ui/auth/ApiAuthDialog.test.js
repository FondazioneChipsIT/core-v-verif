...
describe('ApiAuthDialog', () => {
    ...
    it('renders correctly', async () => {
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(ApiAuthDialog, { onSubmit: onSubmit, onCancel: onCancel }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
        unmount();
    });
    it('renders with a defaultValue', async () => {
        const { waitUntilReady, unmount } = render(_jsx(ApiAuthDialog, { onSubmit: onSubmit, onCancel: onCancel, defaultValue: "test-key" }));
        await waitUntilReady();
    ...
    ])('calls $expectedCall.name when $keyName is pressed', async ({ keyName, sequence, expectedCall, args }) => {
        mockBuffer.text = 'submitted-key'; // Set for the onSubmit case
        const { waitUntilReady, unmount } = render(_jsx(ApiAuthDialog, { onSubmit: onSubmit, onCancel: onCancel }));
        await waitUntilReady();
    ...
    it('displays an error message', async () => {
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(ApiAuthDialog, { onSubmit: onSubmit, onCancel: onCancel, error: "Invalid API Key" }));
        await waitUntilReady();
