    ...
    return useMemo(() => {
        ...
        if (!hasTrailingSpace && currentLevel) {
            ...
            if (!exactMatchAsParent && partial && currentLevel) {
                const prefixParentMatches = currentLevel.filter((cmd) => !!cmd.subCommands &&
                    (cmd.name.toLowerCase().startsWith(partial.toLowerCase()) ||
                        cmd.altNames?.some((alt) => alt.toLowerCase().startsWith(partial.toLowerCase()))));
            ...
            const performFuzzySearch = async () => {
                ...
                if (!signal.aborted) {
                    ...
                    const finalSuggestions = sortedSuggestions.map((cmd) => {
                        const canonicalParentName = parserResult.usedPrefixParentDescent &&
                            leafCommand &&
                    ...
                    const isTopLevelChatOrResumeContext = !!(leafCommand &&
                        (leafCommand.name === 'chat' || leafCommand.name === 'resume') &&
