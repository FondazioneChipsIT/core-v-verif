...
export function useSuspend({ handleWarning, setRawMode, refreshStatic, setForceRerenderKey, shouldUseAlternateScreen, }) {
    const [ctrlZPressCount, setCtrlZPressCount] = useState(0);
    const ctrlZTimerRef = useRef(null);
    const onResumeHandlerRef = useRef(null);
    useEffect(() => () => {
    ...
    useEffect(() => {
        ...
        if (ctrlZPressCount > 1) {
            ...
            handleWarning(`Press ${suspendKey} again to suspend. Undo has moved to ${undoKey}.`);
...
//# sourceMappingURL=useSuspend.js.map