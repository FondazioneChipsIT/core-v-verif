...
const setupMocks = ({ atSuggestions = [], slashSuggestions = [], shellSuggestions = [], isLoading = false, isPerfectMatch = false, slashCompletionRange = { completionStart: 0, completionEnd: 0 }, shellCompletionRange = {
    completionStart: 0,
    completionEnd: 0,
    query: '',
    activeStart: 0,
...
describe('useCommandCompletion', () => {
    ...
    const renderCommandCompletionHook = (initialText, cursorOffset, shellModeActive = false, active = true) => {
        const renderResult = renderWithProviders(_jsx(TestComponent, { initialText: initialText, cursorOffset: cursorOffset, shellModeActive: shellModeActive, active: active }));
        return {
            result: {
                get current() {
                    return hookResult;
    ...
    describe('handleAutocomplete', () => {
        ...
        it('should show ghost text for a single shell completion', async () => {
            ...
            const { result } = renderCommandCompletionHook(text, text.length, true);
            await waitFor(() => {
                expect(result.current.isLoadingSuggestions).toBe(false);
            });
        ...
        it('should not show ghost text if the typed text extends past the completion', async () => {
            ...
            const { result } = renderCommandCompletionHook(text, cursorOffset, true);
            await waitFor(() => {
                expect(result.current.isLoadingSuggestions).toBe(false);
            });
            expect(result.current.promptCompletion.text).toBe('');
        ...
        it('should clear ghost text after user types a space when exact match ghost text was showing', async () => {
            ...
            const { result } = renderCommandCompletionHook(textWithoutSpace, textWithoutSpace.length, true);
            await waitFor(() => {
                expect(result.current.isLoadingSuggestions).toBe(false);
            });
