 ...
 */
const findNextValidIndex = (currentIndex, direction, items, wrapAround = true) => {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (!wrapAround) {
            ...
            if ((direction === 'down' && nextIndex === len - 1) ||
 ...
 */
export function useSelectionList({ items, initialIndex = 0, onSelect, onHighlight, isFocused = true, showNumbers = false, wrapAround = true, focusKey, priority, }) {
    ...
    useEffect(() => {
        ...
        if (focusKey === lastProcessedFocusKeyRef.current)
            return;
