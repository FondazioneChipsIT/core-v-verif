 ...
 */
export async function handleCreditsFlow(args) {
...
// Overage menu flow
// ---------------------------------------------------------------------------
async function handleOverageMenu(args, creditBalance) {
    const { config, fallbackModel, usageLimitReachedModel, overageStrategy, resetTime, isDialogPending, setOverageMenuRequest, setModelSwitchedFromQuotaError, } = args;
    logBillingEvent(config, new OverageMenuShownEvent(usageLimitReachedModel, creditBalance, overageStrategy));
    ...
    const overageIntent = await new Promise((resolve) => {
    ...
    logOverageOptionSelected(config, usageLimitReachedModel, overageIntent, creditBalance);
    switch (overageIntent) {
        ...
        case 'manage': {
            ...
            const manageUrl = await openG1Url('activity', G1_UTM_CAMPAIGNS.MANAGE_ACTIVITY);
            if (manageUrl) {
...
// Empty wallet flow
// ---------------------------------------------------------------------------
async function handleEmptyWalletMenu(args) {
    const { config, fallbackModel, usageLimitReachedModel, resetTime, isDialogPending, setEmptyWalletRequest, setModelSwitchedFromQuotaError, } = args;
    logBillingEvent(config, new EmptyWalletMenuShownEvent(usageLimitReachedModel));
    if (isDialogPending.current) {
        return 'stop';
    }
    isDialogPending.current = true;
    setModelSwitchedFromQuotaError(true);
    config.setQuotaErrorOccurred(true);
    const emptyWalletIntent = await new Promise((resolve) => {
        setEmptyWalletRequest({
            failedModel: usageLimitReachedModel,
            fallbackModel,
            resetTime,
            onGetCredits: async () => {
                logCreditPurchaseClick(config, 'empty_wallet_menu', usageLimitReachedModel);
                const creditsUrl = await openG1Url('credits', G1_UTM_CAMPAIGNS.EMPTY_WALLET_ADD_CREDITS);
                if (creditsUrl) {
                    args.historyManager.addItem({
                        type: MessageType.INFO,
...
// Telemetry helpers
// ---------------------------------------------------------------------------
function logOverageOptionSelected(config, model, option, creditBalance) {
    logBillingEvent(config, new OverageOptionSelectedEvent(model, option, creditBalance));
    recordOverageOptionSelected(config, {
        selected_option: option,
        model,
    });
}
function logCreditPurchaseClick(config, source, model) {
    logBillingEvent(config, new CreditPurchaseClickEvent(source, model));
    recordCreditPurchaseClick(config, { source, model });
}
async function openG1Url(path, campaign) {
    try {
...
//# sourceMappingURL=creditsFlowHandler.js.map