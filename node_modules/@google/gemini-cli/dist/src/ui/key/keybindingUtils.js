...
export function formatKeyBinding(binding, platform) {
    ...
    const parts = [];
    if (binding.ctrl)
        parts.push(modMap.ctrl);
    if (binding.alt)
        parts.push(modMap.alt);
    if (binding.shift)
        parts.push(modMap.shift);
    if (binding.cmd)
        parts.push(modMap.cmd);
 ...
 */
export function formatCommand(command, config = defaultKeyBindings, platform) {
