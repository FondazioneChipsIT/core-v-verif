...
import { defaultKeyMatchers, Command, createKeyMatchers, } from './keyMatchers.js';
        ...
        testCases.forEach(({ command, positive, negative }) => {
            it(`should match ${command} correctly`, () => {
                positive.forEach((key) => {
                    expect(defaultKeyMatchers[command](key), `Expected ${command} to match ${JSON.stringify(key)}`).toBe(true);
                });
                negative.forEach((key) => {
                    expect(defaultKeyMatchers[command](key), `Expected ${command} to NOT match ${JSON.stringify(key)}`).toBe(false);
                });
            });
        });
    });
