...
import { ApprovalMode, coreEvents, debugLogger, processSingleFileContent, partToString, readFileWithEncoding, } from '@google/gemini-cli-core';
...
async function copyAction(context) {
    ...
    try {
        ...
        coreEvents.emitFeedback('info', `Plan copied to clipboard (${path.basename(planPath)}).`);
    }
    catch (error) {
...
export const planCommand = {
    ...
    action: async (context) => {
        ...
        try {
            const content = await processSingleFileContent(approvedPlanPath, config.storage.getPlansDir(), config.getFileSystemService());
