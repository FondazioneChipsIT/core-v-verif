...
describe('hooksCommand', () => {
    ...
    describe('enable subcommand', () => {
        ...
        it('should enable a hook and update settings', async () => {
            ...
            const result = await enableCmd.action(mockContext, 'test-hook');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.User, 'hooksConfig.disabled', ['other-hook']);
            expect(mockHookSystem.setHookEnabled).toHaveBeenCalledWith('test-hook', true);
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: 'Hook "test-hook" enabled by removing it from the disabled list in user (/mock/user.json) and workspace (/mock/workspace.json) settings.',
            });
        });
        it('should complete hook names using friendly names', () => {
            const enableCmd = hooksCommand.subCommands.find((cmd) => cmd.name === 'enable');
            const hookEntry = createMockHook('./hooks/test.sh', HookEventName.BeforeTool, false);
    ...
    describe('disable subcommand', () => {
        ...
        it('should disable a hook and update settings', async () => {
            ...
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'hooksConfig.disabled', ['test-hook']);
            expect(mockHookSystem.setHookEnabled).toHaveBeenCalledWith('test-hook', false);
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: 'Hook "test-hook" disabled by adding it to the disabled list in workspace (/mock/workspace.json) settings.',
            });
        });
    ...
    describe('enable-all subcommand', () => {
        ...
        it('should enable all disabled hooks', async () => {
            ...
            const result = await enableAllCmd.action(mockContext, '');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(expect.any(String), // enableAll uses legacy logic so it might return 'Workspace' or 'User' depending on ternary
    ...
    describe('disable-all subcommand', () => {
        ...
        it('should disable all enabled hooks', async () => {
            ...
            const result = await disableAllCmd.action(mockContext, '');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(expect.any(String), 'hooksConfig.disabled', ['hook-1', 'hook-2', 'hook-3']);
