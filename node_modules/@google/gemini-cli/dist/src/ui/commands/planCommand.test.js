...
import { ApprovalMode, coreEvents, processSingleFileContent, readFileWithEncoding, } from '@google/gemini-cli-core';
...
describe('planCommand', () => {
    ...
    describe('copy subcommand', () => {
        it('should copy the approved plan to clipboard', async () => {
            ...
            if (!copySubCommand?.action)
                throw new Error('Copy action missing');
            ...
            expect(coreEvents.emitFeedback).toHaveBeenCalledWith('info', 'Plan copied to clipboard (approved-plan.md).');
        });
        it('should warn if no approved plan is found', async () => {
            ...
            if (!copySubCommand?.action)
                throw new Error('Copy action missing');
            await copySubCommand.action(mockContext, '');
            expect(coreEvents.emitFeedback).toHaveBeenCalledWith('warning', 'No approved plan found to copy.');
        });
    });
});
