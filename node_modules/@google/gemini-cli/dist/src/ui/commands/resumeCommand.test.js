...
describe('resumeCommand', () => {
    ...
    it('should expose unified chat subcommands directly under /resume', () => {
        ...
        expect(visibleSubCommandNames).toEqual(expect.arrayContaining(['list', 'save', 'resume', 'delete', 'share']));
    });
    it('should keep a hidden /resume checkpoints compatibility alias', () => {
        const checkpoints = resumeCommand.subCommands?.find((subCommand) => subCommand.name === 'checkpoints');
        expect(checkpoints?.hidden).toBe(true);
        expect(checkpoints?.subCommands?.map((subCommand) => subCommand.name)).toEqual(expect.arrayContaining(['list', 'save', 'resume', 'delete', 'share']));
    });
});
