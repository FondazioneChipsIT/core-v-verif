...
async function installAction(context, args, requestConsentOverride) {
    const extensionLoader = context.services.config?.getExtensionLoader();
    if (!(extensionLoader instanceof ExtensionManager)) {
        debugLogger.error(`Cannot ${context.invocation?.name} extensions in this environment`);
        return;
    ...
    try {
        ...
        const extension = await extensionLoader.installOrUpdateExtension(installMetadata, undefined, requestConsentOverride);
        context.ui.addItem({
            type: MessageType.INFO,
...
async function uninstallAction(context, args) {
    ...
    for (const extensionName of namesToUninstall) {
        ...
        try {
            ...
            context.ui.addItem({
                type: MessageType.ERROR,
                text: `Failed to uninstall extension "${extensionName}": ${getErrorMessage(error)}`,
            });
        }
    }
}
