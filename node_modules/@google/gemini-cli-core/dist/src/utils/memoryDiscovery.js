...
export async function readGeminiMdFiles(filePaths, importFormat = 'tree') {
    ...
    for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
        ...
        const batchPromises = batch.map(async (filePath) => {
            try {
                ...
                const processedResult = await processImports(content, path.dirname(filePath), false, undefined, undefined, importFormat);
...
export async function getEnvironmentMemoryPaths(trustedRoots) {
 ...
 */
export async function loadServerHierarchicalMemory(currentWorkingDirectory, includeDirectoriesToReadGemini, fileService, extensionLoader, folderTrust, importFormat = 'tree', fileFilteringOptions, maxDirs = 200) {
...
export async function refreshServerHierarchicalMemory(config) {
    const result = await loadServerHierarchicalMemory(config.getWorkingDir(), config.shouldLoadMemoryFromIncludeDirectories()
        ...
        : [], config.getFileService(), config.getExtensionLoader(), config.isTrustedFolder(), config.getImportFormat(), config.getFileFilteringOptions(), config.getDiscoveryMaxDirs());
...
export async function loadJitSubdirectoryMemory(targetPath, trustedRoots, alreadyLoadedPaths, alreadyLoadedIdentities) {
