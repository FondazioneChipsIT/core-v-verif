...
import { MCP_TOOL_PREFIX, isMcpToolAnnotation, parseMcpToolName, } from '../tools/mcp-tool.js';
...
function ruleMatches(rule, toolCall, stringifiedArgs, serverName, currentApprovalMode, toolAnnotations, subagent) {
...
export class PolicyEngine {
    ...
    async check(toolCall, serverName, toolAnnotations, subagent) {
        ...
        for (const rule of this.rules) {
            const match = toolCallsToTry.some((tc) => ruleMatches(rule, tc, stringifiedArgs, serverName, this.approvalMode, toolAnnotations, subagent));
            if (match) {
                ...
                if (isShellCommand && toolName) {
                    const shellResult = await this.checkShellCommand(toolName, command, rule.decision, serverName, shellDirPath, rule.allowRedirection, rule, toolAnnotations, subagent);
        ...
        if (decision === undefined) {
            ...
            if (toolName && SHELL_TOOL_NAMES.includes(toolName)) {
                const shellResult = await this.checkShellCommand(toolName, command, this.defaultDecision, serverName, shellDirPath, false, undefined, toolAnnotations, subagent);
                decision = shellResult.decision;
                matchedRule = shellResult.rule;
            }
            else {
        ...
        if (decision !== PolicyDecision.DENY && this.checkerRunner) {
            for (const checkerRule of this.checkers) {
                if (ruleMatches(checkerRule, toolCall, stringifiedArgs, serverName, this.approvalMode, toolAnnotations, subagent)) {
    ...
    getExcludedTools(toolMetadata, allToolNames) {
        ...
        for (const toolName of allToolNames) {
            ...
            for (const rule of this.rules) {
                ...
                serverName, this.approvalMode, annotations);
                if (appliesToTool) {
                    if (rule.argsPattern) {
