 ...
 */
export function getGrpcCredentials(url) {
 ...
 */
export function getGrpcChannelOptions(hostname) {
    return {
 ...
 */
export async function pinUrlToIp(url, agentName) {
    if (!url)
        return { pinnedUrl: url, hostname: '' };
    ...
    try {
        ...
        const sanitizedHost = hostname.startsWith('[') && hostname.endsWith(']')
            ? hostname.slice(1, -1)
            : hostname;
        // Resolve DNS to check the actual target IP and pin it
        const addresses = await lookup(hostname, { all: true });
        const publicAddresses = addresses.filter((addr) => !isAddressPrivate(addr.address) ||
            sanitizedHost === 'localhost' ||
            sanitizedHost === '127.0.0.1' ||
            sanitizedHost === '::1');
        if (publicAddresses.length === 0) {
            if (addresses.length > 0) {
                throw new Error(`Refusing to load agent '${agentName}': transport URL '${url}' resolves to private IP range.`);
            }
            throw new Error(`Failed to resolve any public IP addresses for host: ${hostname}`);
        }
        ...
        if (e instanceof Error && e.message.includes('Refusing'))
            throw e;
 ...
 */
export function splitAgentCardUrl(url) {
    ...
    try {
        ...
        if (parsedUrl.pathname.endsWith(standardPath)) {
            ...
            parsedUrl.pathname = parsedUrl.pathname.substring(0, parsedUrl.pathname.lastIndexOf(standardPath));
            parsedUrl.search = '';
            parsedUrl.hash = '';
 ...
 */
function extractNormalizedInterfaces(card) {
    const rawInterfaces = getArray(card, 'additionalInterfaces') ||
