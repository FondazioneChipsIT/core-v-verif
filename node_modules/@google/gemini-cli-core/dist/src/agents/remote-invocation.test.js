...
describe('RemoteAgentInvocation', () => {
    ...
    describe('Execution Logic', () => {
        it('should lazy load the agent without auth handler when no auth configured', async () => {
            ...
            await invocation.execute(new AbortController().signal);
            expect(mockClientManager.loadAgent).toHaveBeenCalledWith('test-agent', 'http://test-agent/card', undefined);
        });
        it('should use A2AAuthProviderFactory when auth is present in definition', async () => {
            ...
            const invocation = new RemoteAgentInvocation(authDefinition, { query: 'hi' }, mockMessageBus);
            await invocation.execute(new AbortController().signal);
            ...
            expect(mockClientManager.loadAgent).toHaveBeenCalledWith('test-agent', 'http://test-agent/card', mockHandler);
        });
        it('should return error when auth provider factory returns undefined for configured auth', async () => {
            ...
            expect(result.error?.message).toContain("Failed to create auth provider for agent 'test-agent'");
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should use generic message for non-A2AAgentError errors', async () => {
            ...
            const result = await invocation.execute(new AbortController().signal);
            expect(result.error).toBeDefined();
            expect(result.returnDisplay).toContain('Error calling remote agent: something unexpected');
        });
