...
describe('BrowserAgentInvocation', () => {
    ...
    describe('execute', () => {
        ...
        it('should work without updateOutput (fire-and-forget)', async () => {
            ...
            await expect(invocation.execute(controller.signal)).resolves.toBeDefined();
        });
        ...
        it('should handle THOUGHT_CHUNK and emit structured progress', async () => {
            ...
            const updateOutput = vi.fn();
            const invocation = new BrowserAgentInvocation(mockConfig, mockParams, mockMessageBus);
            const executePromise = invocation.execute(new AbortController().signal, updateOutput);
        ...
        it('should mark all running tools as errored when ERROR has no callId', async () => {
            ...
            const progressCalls = updateOutput.mock.calls
                ...
                .filter((p) => p.isSubagentProgress);
            const allItems = progressCalls.flatMap((p) => p.recentActivity);
            const toolA = allItems.find((a) => a.type === 'tool_call' && a.content === 'tool_a');
            const toolB = allItems.find((a) => a.type === 'tool_call' && a.content === 'tool_b');
