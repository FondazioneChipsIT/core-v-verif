...
export async function updatePolicy(tool, outcome, confirmationDetails, deps) {
    ...
    if (outcome === ToolConfirmationOutcome.ProceedAlwaysAndSave) {
        ...
        if (deps.config.isTrustedFolder() &&
    ...
    if (confirmationDetails?.type === 'mcp') {
        await handleMcpPolicyUpdate(tool, outcome, confirmationDetails, deps.messageBus, persistScope);
        return;
    }
    // Generic Fallback (Shell, Info, etc.)
    await handleStandardPolicyUpdate(tool, outcome, confirmationDetails, deps.messageBus, persistScope, deps.toolInvocation, deps.config);
}
/**
 ...
 */
async function handleStandardPolicyUpdate(tool, outcome, confirmationDetails, messageBus, persistScope, toolInvocation, config) {
