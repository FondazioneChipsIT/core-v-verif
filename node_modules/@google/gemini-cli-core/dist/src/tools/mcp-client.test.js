...
import { PromptListChangedNotificationSchema, ResourceListChangedNotificationSchema, ToolListChangedNotificationSchema, ProgressNotificationSchema, } from '@modelcontextprotocol/sdk/types.js';
...
describe('mcp-client', () => {
    ...
    describe('Dynamic Tool Updates', () => {
        it('should set up notification handler if server supports tool list changes', async () => {
            ...
            const client = new McpClient('test-server', { command: 'test-command' }, mockedToolRegistry, {
                getPromptsByServer: vi.fn().mockReturnValue([]),
                registerPrompt: vi.fn(),
            }, {
                getResourcesByServer: vi.fn().mockReturnValue([]),
        ...
        it('should handle concurrent updates from multiple servers', async () => {
            ...
            const onContextUpdatedSpy = vi.fn().mockResolvedValue(undefined);
            const clientA = new McpClient('server-A', { command: 'cmd-a' }, mockedToolRegistry, {}, {}, workspaceContext, MOCK_CONTEXT, false, '0.0.1', onContextUpdatedSpy);
            const clientB = new McpClient('server-B', { command: 'cmd-b' }, mockedToolRegistry, {}, {}, workspaceContext, MOCK_CONTEXT, false, '0.0.1', onContextUpdatedSpy);
        ...
        it('should abort discovery and log error if timeout is exceeded during refresh', async () => {
            ...
            { command: 'test-command', timeout: 50 }, mockedToolRegistry, {
                getPromptsByServer: vi.fn().mockReturnValue([]),
                registerPrompt: vi.fn(),
                removePromptsByServer: vi.fn(),
            }, {
