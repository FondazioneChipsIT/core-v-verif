...
## GVSOC as DUT (ISS qualification):

Besides the RTL flow above, `config_gvsoc.ini` runs the same arch-test suite with the
**GVSOC ISS as the DUT** and Sail as the golden reference. This qualifies the reference
model used by the co-simulation independently of the RTL (no Questa involved: the tests
run on the standalone `cv32e40p-v2-standalone[-fpu]` targets).

- One-time setup (site-specific paths are documented in the header of `config_gvsoc.ini`):
  generate the Sail configuration with `sail_cSim/gen_sail_config.py`, make sure
  `sail_riscv_sim` and `riscv32-unknown-elf-*` resolve, and point `gvrun`/`env_bin`
  to the local GVSOC install.
- Run: `riscof run --config=config_gvsoc.ini --suite=<suite>/riscv-test-suite/ --env=<suite>/riscv-test-suite/env`
  from this directory (or through the usual make targets with `RISCOF_CONFIG_FILE=config_gvsoc.ini`).
- The suite is pinned to **3.9.1** in `ExternalRepos.mk`: release 4.0.0 switched to the
  CTP framework and no longer ships `riscv-test-suite/rv32i_m`, so `TAG=latest` breaks
  the plugins.
- The sail_cSim plugin targets sail-riscv >= 0.13 (single `sail_riscv_sim` binary,
  configured through the JSON given by the mandatory `sail_config` key). The prebuilt
  release binaries run on RHEL8/glibc 2.28 — no local build needed.
- Signature extraction: `RVMODEL_HALT` streams the signature words through an undecoded
  offset of the exit device and `gvsoc/sig_extract.py` recovers them from the debug
  trace. The device's SIG_START/SIG_END/SIG_WRITE registers are still stubs upstream;
  implementing them removes the trace dependency (planned follow-up).
